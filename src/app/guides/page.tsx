...
export default function GuidesPage() {
  ...
  const initialExam = (["toefl", "ielts", "toeic"] as string[]).includes(
    selectedExam,
  )
    ? (selectedExam as GuideExam)
    : "all";
  ...
  const filtered = useMemo(
    () =>
      guides.filter(
        (guide) =>
          (exam === "all" || guide.exam === "all" || guide.exam === exam) &&
          `${guide.title} ${guide.summary} ${GUIDE_CATEGORY_LABELS[guide.category]}`
            .toLowerCase()
  ...
  const grouped = useMemo(
    () =>
      Object.entries(GUIDE_CATEGORY_LABELS)
        .map(([id, label]) => ({
          id,
          label,
          guides: filtered.filter((guide) => guide.category === id),
        }))
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-10">
        <header className="mb-8 animate-in fade-in slide-in-from-bottom-3 duration-700">
          ...
          <p className="mt-2 max-w-2xl text-slate-600 dark:text-slate-300">
            新しいトレーニング、学習データの見方を試験・技能ごとに確認できます。
          </p>
        </header>
        <Link
          href="/home?tour=1"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block font-bold text-slate-900 dark:text-white">
              初回ログイン時のステップガイドをもう一度見る
            </span>
            <span className="mt-1 block text-sm text-slate-600 dark:text-slate-300">
              Home上で、試験切替・技能・問題タイプなどの基本操作を順番に確認できます。
            </span>
          </span>
          ...
          <div className="flex flex-wrap gap-2">
            {(Object.keys(GUIDE_EXAM_LABELS) as GuideExam[]).map((id) => (
              <button
                key={id}
                onClick={() => setExam(id)}
                className={`rounded-full px-4 py-2 text-sm font-medium transition ${exam === id ? "bg-eg text-black" : "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300"}`}
              >
                {id === "all" ? "すべて" : GUIDE_EXAM_LABELS[id]}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-10">
                    ...
                    >
                      <div className="mb-3 flex flex-wrap gap-2">
                        <Badge variant="outline">
                          {GUIDE_EXAM_LABELS[guide.exam]}
                        </Badge>
                        <Badge variant="secondary">
                          {GUIDE_SKILL_LABELS[guide.skill]}
                        </Badge>
                      </div>
                      <h3 className="text-lg font-bold text-slate-900 group-hover:text-eg-dark dark:text-white">
                        {guide.title}
                      </h3>
                      <p className="mt-2 flex-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
                        {guide.summary}
                      </p>
                      <span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-eg-dark">
                        ガイドを読む
            ...
            {!grouped.length && (
              <div className="rounded-2xl border border-dashed py-20 text-center text-slate-500">
                該当するガイドはありません。
              </div>
            )}
          </div>
