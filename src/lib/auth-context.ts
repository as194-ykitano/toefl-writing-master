...
// PrepMaster 側の AuthContext は { user, loading, logout } のみなので、
// ここで isAdmin（email allowlist 判定）と studentData（PrepMaster にコーチ概念は無いので undefined）
// を合成して同じ形にそろえる。
import { useAuth as useBaseAuth } from "@/contexts/AuthContext";
import { isAdmin as isAdminEmail } from "@/lib/utils";

export type StudentData = {
  /** PrepMaster にはコーチ割当が無いため常に undefined */
  coach?: string;
};
